...
export default class AuthEpic {
    ...
    static signupEpic = (action$: ActionsObservable<any>) =>
        action$.ofType(AuthActions.SIGNUP)
            ...
            .switchMap(({payload}) => {
                return Observable.fromPromise(AuthEpic.mainRef.child(`users/${payload.userId}`).once('value'))
                    .map(snapshot => {
                        if (snapshot.val()) {
      ...
      static signInWithFacebook = (action$: ActionsObservable<any>) =>
        action$.ofType(AuthActions.FBLOGIN)
            .switchMap(() => {
                ...
                return Observable.fromPromise(firebase.auth().signInWithPopup(provider))
                    .map(snapshot => {
                        if (snapshot) {
                             console.log('fbUser does exist',snapshot);
                       ...
                       // console.log('ok created user wow!'+obj)
                        ...
                        return {
                            ...
                            payload: uObj
                             ...
                             console.log('User does not exist',snapshot);
                            // return {
                            //     type: AuthActions.CREATEUSER,
                            //     payload
                            // }
